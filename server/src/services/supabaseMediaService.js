...
class SupabaseMediaService {
    ...
    async getVideosForAgent(agentId, farmId, accessLevel = 'viewer') {
        try {
            if (!this.supabase) {
                logger.warn('Supabase not configured, returning empty videos');
                return [];
            }
    ...
    async getPhotosForAgent(agentId, farmId, accessLevel = 'viewer') {
        try {
            if (!this.supabase) {
                logger.warn('Supabase not configured, returning empty photos');
                return [];
            }
    ...
    async getSignedVideoUrl(videoId, agentId, expiresIn = 3600) {
        try {
            ...
            if (video.agent_access_level === 'private' && !(await this.hasAdminAccess(agentId))) {
    ...
    async getSignedPhotoUrl(photoId, agentId, expiresIn = 3600) {
        try {
            ...
            // Check access permissions
            if (photo.agent_access_level === 'private' && !(await this.hasAdminAccess(agentId))) {
                throw new Error('Access denied');
            }
    ...
    async deleteVideo(videoId, adminId) {
        try {
            if (!(await this.hasAdminAccess(adminId))) {
    ...
    async hasAdminAccess(agentId) {
        try {
            if (!this.supabase || !agentId) {
                return false;
            }
            ...
            if (error || !user) {
                logger.warn('User not found or error checking access:', { agentId, error });
                return false;
            }
