# Agent OS Base Installation Script - Simple Working Version
...
if ($Help) {
    Write-Host ""
    Write-Host "Usage: .\agent-os-setup-simple-working.ps1 [OPTIONS]"
    Write-Host ""
    Write-Host "Options:"
...
Write-Host ""
Write-Host "🚀 Agent OS Base Installation - Simple Working Version"
Write-Host "====================================================="
Write-Host ""
...
Write-Host "📍 The Agent OS base installation will be installed in the current directory ($CURRENT_DIR)"
Write-Host ""
...
function Save-File {
    ...
    if ((Test-Path $OutputPath) -and !$Overwrite) {
        Write-Host "  ⏭️  Skipping $Description (already exists)"
        return $true
    }
...
Save-File -Url $functionsUrl -OutputPath $functionsPath -Overwrite $true -Description "setup/functions.sh"
...
Save-File -Url $instructionsUrl -OutputPath $instructionsPath -Overwrite $OverwriteInstructions -Description "instructions/README.md"
...
Save-File -Url $standardsUrl -OutputPath $standardsPath -Overwrite $OverwriteStandards -Description "standards/README.md"
...
Save-File -Url $commandsUrl -OutputPath $commandsPath -Overwrite $true -Description "commands/README.md"
...
Save-File -Url $configUrl -OutputPath $configPath -Overwrite $OverwriteConfig -Description "config.yml"
...
Save-File -Url $projectUrl -OutputPath $projectPath -Overwrite $true -Description "setup/project.sh"
...
if ($ClaudeCode) {
    ...
    foreach ($agent in $agents) {
        ...
        Save-File -Url $agentUrl -OutputPath $agentPath -Overwrite $false -Description "claude-code/agents/$agent.md"
    }
    ...
    if (Test-Path $configPath) {
        try {
            ...
            Write-Host "  ⚠️  Could not update config.yml, please manually enable claude_code"
        }
    }
}
...
if ($Cursor) {
    ...
    if (Test-Path $configPath) {
        try {
            ...
            Write-Host "  ⚠️  Could not update config.yml, please manually enable cursor"
        }
    }
}
...
if (!(Test-Path $configPath)) {
    ...
    # Create config content using a simple approach
    $configLines = @()
    $configLines += "# Agent OS Configuration"
    $configLines += "version: `"1.0.0`""
    $configLines += ""
    $configLines += "# Project types"
    $configLines += "project_types:"
    $configLines += "  - name: `"python`""
    $configLines += "    description: `"Python projects`""
    $configLines += "    enabled: true"
    $configLines += "  - name: `"javascript`""
    $configLines += "    description: `"JavaScript/Node.js projects`""
    $configLines += "    enabled: true"
    $configLines += "  - name: `"react`""
    $configLines += "    description: `"React applications`""
    $configLines += "    enabled: true"
    $configLines += ""
    $configLines += "# Features"
    $configLines += "features:"
    $configLines += "  claude_code:"
    $configLines += "    enabled: false"
    $configLines += "  cursor:"
    $configLines += "    enabled: false"
    $configLines += ""
    $configLines += "# Standards"
    $configLines += "standards:"
    $configLines += "  python:"
    $configLines += "    enabled: true"
    $configLines += "    file: `"python-style.md`""
    $configLines += "  javascript:"
    $configLines += "    enabled: true"
    $configLines += "    file: `"javascript-style.md`""
    $configLines += "  react:"
    $configLines += "    enabled: true"
    $configLines += "    file: `"react-style.md`""
