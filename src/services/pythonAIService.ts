...
export class PythonAIService {
  // Use ApiClient base '/api' + here '/python-ai' → '/api/python-ai'
  private baseUrl = '/python-ai';
  ...
  async runDailyOperations(inputs: DailyOperationsInput = {}): Promise<PythonAIResponse> {
    try {
      const response = await apiClient.post<{ status: string; output?: string }>(`${this.baseUrl}/daily-operations`, inputs);
      return {
  ...
  async runCrisisResponse(inputs: CrisisResponseInput = {}): Promise<PythonAIResponse> {
    try {
      const response = await apiClient.post<{ status: string; output?: string }>(`${this.baseUrl}/crisis-response`, inputs);
      return {
        success: response.status === 'success',
        output: response.output || '',
        operation: 'crisis',
  ...
  async runContentCreation(inputs: ContentCreationInput = {}): Promise<PythonAIResponse> {
    try {
      const response = await apiClient.post<{ status: string; output?: string }>(`${this.baseUrl}/content-creation`, inputs);
      return {
        success: response.status === 'success',
        output: response.output || '',
        operation: 'content',
  ...
  async runStrategicPlanning(inputs: StrategicPlanningInput = {}): Promise<PythonAIResponse> {
    try {
      const response = await apiClient.post<{ status: string; output?: string }>(`${this.baseUrl}/strategic-planning`, inputs);
      return {
        success: response.status === 'success',
        output: response.output || '',
        operation: 'strategic',
  ...
  async runFullCrew(inputs: Record<string, unknown> = {}): Promise<PythonAIResponse> {
    try {
      const response = await apiClient.post<{ status: string; output?: string }>(`${this.baseUrl}/full-crew`, inputs);
      return {
        success: response.status === 'success',
        output: response.output || '',
        operation: 'full',
  ...
  async runTest(inputs: Record<string, unknown> = {}): Promise<PythonAIResponse> {
    try {
      const response = await apiClient.post<{ status: string; output?: string }>(`${this.baseUrl}/test`, inputs);
      return {
        success: response.status === 'success',
        output: response.output || '',
        operation: 'test',
